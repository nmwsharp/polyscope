...
// Keep track of global structure colors
int iStructureColor = 0;
} // namespace
...
Color3f SubColorManager::getNextSubColor(std::string key) {
  ...
  // If we haven't seen the key before, assign a new index
  if (assignedColors.find(key) == assignedColors.end()) {
    int newInd = assignedColors.size() + 1;
    assignedColors[key] = newInd;
  }

  return indexOffsetHue(baseColor, assignedColors[key]);
}
