...
bool messageIsBlockingScreen() {
  return currentlyShowingError || warningMessages.size() > 0;
}

void info(std::string message) {
  cout << options::printPrefix << message << endl;
}
...
void buildMessagesUI() {
  ...
  if(currentlyShowingError) {
    ...
    if (ImGui::BeginPopupModal("ERROR", NULL, ImGuiWindowFlags_NoMove)) {
  ...
  if(!currentlyShowingError && warningMessages.size() > 0) {
    ...
    ImVec2 warningModalSize(std::max(view::windowWidth/5.0f, std::min(warningMaxTextSize.x+50, view::windowWidth/2.0f)), 0);
