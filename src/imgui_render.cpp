...
// ImGui (via setting up 'RenderDrawListsFn' in the ImGuiIO structure) Note that
// this implementation is little overcomplicated because we are saving/setting
// up/restoring every OpenGL state explicitly, in order to be able to run within
// any OpenGL engine that doesn't do so. If text or lines are blurry when
// integrating ImGui in your engine: in your Render function, try translating
...
bool ImGui_ImplGlfwGL3_Init(GLFWwindow* window, bool install_callbacks) {
  ...
  io.ClipboardUserData = mainWindow;
#ifdef _WIN32
  io.ImeWindowHandle = glfwGetWin32Window(mainWindow);
#endif
...
void ImGui_ImplGlfwGL3_NewFrame() {
  ...
  if (glfwGetWindowAttrib(mainWindow, GLFW_FOCUSED)) {
    if (io.WantSetMousePos) {
      glfwSetCursorPos(mainWindow, (double)io.MousePos.x,
    ...
    io.MousePos = ImVec2(
        ...
        -std::numeric_limits<float>::max());  // tweaked by nice from FLT_MAX
  }
  ...
  for (int i = 0; i < 3; i++) {
    ...
    io.MouseDown[i] =
        g_MouseJustPressed[i] || glfwGetMouseButton(mainWindow, i) != 0;
