...
#include "polyscope/render/opengl/egl_gl_engine.h"
...
void checkEGLError(bool fatal = true) {
  ...
  while (err != EGL_SUCCESS) {
    ...
    case EGL_BAD_CURRENT_SURFACE:
      errText = "The current surface of the calling thread is a window, pixel buffer or pixmap that is no longer "
                "valid.";
      break;
    ...
    case EGL_BAD_SURFACE:
      errText = "An EGLSurface argument does not name a valid surface (window, pixel buffer or pixmap) configured for "
                "GL rendering.";
      break;
    ...
    if (polyscope::options::verbosity > 0) {
      std::cout << polyscope::options::printPrefix << "Polyscope EGL Error!  Text: " << errText << std::endl;
    }
    if (fatal) {
      exception("EGL error occurred. Text: " + errText);
    }

    err = eglGetError(); // get the next error
  }
}
...
// Load openGL functions (using GLAD)
  ...
  if (options::verbosity > 0) {
    std::cout << options::printPrefix << "Backend: openGL3_glfw_egl -- "
