...
DepthRenderImage* renderImplictSurfaceSphereMarch(QuantityStructure<S>* parent, std::string name, Func&& func,
  ...
  return renderImplictSurfaceSphereMarchBatch(parent, name, batchFunc, opts);
}
...
DepthRenderImage* renderImplictSurfaceSphereMarchBatch(QuantityStructure<S>* parent, std::string name, Func&& func,
  ...
  float missDist = opts.missDist.asAbsolute();
  float hitDist = opts.hitDist.asAbsolute();
  float stepFactor = opts.stepFactor;
  size_t nMaxSteps = opts.nMaxSteps;
  float normalSampleEps = opts.normalSampleEps;
  ...
  for (size_t iY = 0; iY < dimY; iY++) {
    for (size_t iX = 0; iX < dimX; iX++) {
      ...
      rayDirs[ind] = view::screenCoordsToWorldRay(glm::vec2{iX, iY});
      rayInds[ind] = ind;
    }
  }

  std::cout << "tracing " << dimX << " x " << dimY << " rays to render" << std::endl;
  ...
  // Sphere march along the ray to compute depth
  ...
  for (size_t iStep = 0; iStep < nMaxSteps; iStep++) {
    ...
    for (size_t iP = 0; iP < rayDepth.size(); iP++) {
      ...
      bool terminated = missTerminated || (std::abs(currVals[iP]) < hitDist) ||
                        (std::signbit(currVals[iP]) != initSigns[iP]);
      ...
      if (terminated) {
        ...
        // Take a step
        float newDepth = rayDepth[iP] + std::abs(currVals[iP]) * stepFactor;
        glm::vec3 newPos = rayRoots[iP] + newDepth * rayDirs[iP];
        
        //std::cout << "working ray " << iP << " step " << iStep << " has depth "
                  //<< newDepth << " after sdf eval " << currVals[iP] << std::endl;
  ...
  // Add the actual image as a quantity
  if (parent == nullptr) {
    parent = getGlobalFloatingQuantityStructure();
  }
  ...
  return parent->addDepthRenderImageImpl(name, dimX, dimY, rayDepthOut, normalOut);
}
